...
* 中枢默认租约 `DEFAULT_LEASE_SECONDS = 300`（5 分钟），上限 3600。
  长任务必须显式调 `task_heartbeat()` 续租。租约过期的语义是**别的节点可以来抢**，
  不是**你自己不能续**：只要任务还没被回收或被他人接管，原 owner 仍可续租与完成。
* 一旦任务被回收或接管，后续 heartbeat/complete 会返回 400 并带 `error_code`，
  **必须按 error_code 分支处理，不要只看 HTTP 400**：

  ==================  =====================================================
  ``lease_reaped``    租约过期、任务已回池、还没人接手 -> 可以重新 claim 继续干
  ``task_taken_over`` 已被别的节点接管 -> 立刻停手，你产出的东西会被丢弃
  ``lease_released``  自己已经 complete/fail 过了 -> 无事可做，别重试
  ``invalid_lease``   既不是 owner、token 也不对 -> 真·非法调用
  ==================  =====================================================

  此外，当别的节点接管了你过期的任务时，中枢会主动给你发一条私信（kind=notice），
  不必等到下一次心跳被拒才知道。
